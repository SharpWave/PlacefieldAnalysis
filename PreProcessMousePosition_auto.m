...
old_time = time;
[Ypix,time] = resample(Ypix,old_time,1/0.0333);
[Xpix,time] = resample(Xpix,old_time,1/0.0333);

for i = 1:length(time)
    fnum = findclosest(time(i),old_time);
    objTime(i) = fnum/aviSR;
end
...
while (strcmp(MorePoints,'y')) || strcmp(MorePoints,'m') || isempty(MorePoints)
    ...
    if strcmp(MorePoints,'y')
        if auto_thresh_flag == 0 || isempty(epoch_start)
            ...
            while (FrameSelOK == 0)
                ...
                if (sFrame > length(time) || eFrame > length(time))
        ...
        obj.currentTime = objTime(sFrame); % sFrame is the correct frame #, but .avi reads are done according to time
        v = readFrame(obj);
        ...
        for i = frame_use_index
            ...
            %framesToCorrect(i*2);
            obj.currentTime = objTime(framesToCorrect(i*2));
            v = readFrame(obj);
            ...
            elseif length(MouseBlob)>1
                ...
                previousX = xAVI(sFrame+i*2-2);
                previousY = yAVI(sFrame+i*2-2);
                ...
                %previous frame.
                clear mdist;
                for j1 = 1:length(putativeMouseX)
                    mdist(j1) = sqrt((putativeMouseX(j1)-previousX)^2+(putativeMouseY(j1)-previousY)^2);
                end
                
                [mindist,whichMouse] = min(mdist);mindist,

                
                %If they agree, use that blob.
                if mindist < distthresh
                 xm = stats(MouseBlob(whichMouse)).Centroid(1);
                 ym = stats(MouseBlob(whichMouse)).Centroid(2)
                else
                    %keyboard;
                    [xm,ym] = ginput(1);
                end
    ...
    if (strcmp(MorePoints,'g'))
        ...
        for i = 1:length(time)
            obj.currentTime = objTime(i); % sFrame is the correct frame #, but .avi reads are done according to time
            v = readFrame(obj);
            figure(6156);
            imagesc(flipud(v));hold on;
